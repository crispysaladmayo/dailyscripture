...
| ID | Requirement |
|----|-------------|
| FR-C1 | **Today** reflects **primary child** only. **Greeting (M1, Indonesian):** time-appropriate **“Selamat [pagi \| siang \| sore \| malam], Mama [ChildFirstName]”** (clock buckets: before 11 pagi, 11–14 siang, 15–18 sore, else malam) using the **primary child’s first name** from the child profile (FR-B1)—no extra caregiver display-name field required. **Inclusive / locale backlog:** neutral alternatives (e.g. “Orang tua,” “Papa”) and [`suppa-brand-framework.md`](./suppa-brand-framework.md) opt-in labels are **post-M1** unless PO extends locale. |
...
- **Q6 — Today greeting (M1):** **“Selamat [pagi \| siang \| sore \| malam], Mama [ChildFirstName]”** using **primary child’s first name** (existing profile PII per FR-B1). No separate caregiver **display name** field required for greeting.
